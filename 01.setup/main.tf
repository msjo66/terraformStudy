resource "azurerm_resource_group" "rg-az01-msjo66-tf" {
  location = "koreacentral"
  name     = "rg-az01-msjo66-tf"
}

resource "azurerm_virtual_network" "vnet-az01-msjo66-tf" {
  address_space       = ["10.0.0.0/16"]
  location            = azurerm_resource_group.rg-az01-msjo66-tf.location
  name                = "vnet-az01-msjo66-tf"
  resource_group_name = azurerm_resource_group.rg-az01-msjo66-tf.name
  depends_on = [
    azurerm_resource_group.rg-az01-msjo66-tf
  ]
}
...
resource "azurerm_storage_account" "msjo66sttfstateimport" { #이름이 이상한데, storage account의 특징이다. global unique, - 못씀 등등..
    account_replication_type = "LRS"
    account_tier = "Standard"
    location = azurerm_resource_group.rg-az01-msjo66-tf.location
    name = "msjo66sttfstateimport"
    resource_group_name = azurerm_resource_group.rg-az01-msjo66-tf.name
}
resource "azurerm_storage_container" "tfstate" {
    name = "tfstate"
    storage_account_name = azurerm_storage_account.msjo66sttfstateimport.name
